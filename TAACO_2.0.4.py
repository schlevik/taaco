...
class MyApp:
	def __init__(self, parent):
		...
		self.source_text_list = ["null", self.st1_var, self.st2_var,self.st3_var,self.st4_var]
	...
	def runprogram(self, event):
		self.poll(self.progress)
		for x in ['dirname', 'outdirname', 'summary_name']:
			print(x, getattr(self, x))
		ctr = 1
		print(len(self.var_list))
		for i, var in enumerate(self.var_list[1:]):
			try:
				checkbox = self.box_list[i].cget('text')
			except:
				checkbox = "NOT IN LIST"
			print(ctr, str(var), var.get(), checkbox)
			ctr += 1
		print(len(self.source_text_list))
		ctr = 1
		for i ,var in enumerate(self.source_text_list[1:]):
			print(ctr, str(var), var.get())
			ctr += 1
...
def main(indir, outdir, source_text, var_list, source_list):
	...
	if indir is "":
		...
		def dicter(spread_name,delimiter, lower = False):
			...
			if lower == True:
				spreadsheet = file(resource_path(spread_name), "rU").read().lower().split("\n")
		...
		adj_word_list = file("adj_lem_list.txt").read().split("\n")[:-1]
		...
		if source_text != "" and source_options[1] == 1:
			...
			dataQueue.put("Loading COCA Lemma Data...")
			root.update_idletasks()

			mag_news_uni_list = file(resource_path('mag_news_word_list_lemma_freq.csv'), 'rU').read()
			mag_news_uni_F = dict_builder(mag_news_uni_list, 2)

			mag_news_bi_list = file(resource_path('mag_news_bi_list_lemma_freq.csv'), 'rU').read()
			mag_news_bi_F = dict_builder(mag_news_bi_list, 2)

			mag_news_tri_list = file(resource_path('mag_news_tri_list_lemma_freq.csv'), 'rU').read()
			mag_news_tri_F = dict_builder(mag_news_tri_list, 2)

			mag_news_quad_list = file(resource_path('mag_news_quad_list_lemma_freq.csv'), 'rU').read()
			mag_news_quad_F = dict_builder(mag_news_quad_list, 2)

			#noun
			mag_news_n_bi_list = file(resource_path('mag_news_n_list_bi_lemma_freq.csv'), 'rU').read()
			mag_news_n_bi_F = dict_builder(mag_news_n_bi_list, 2)

			mag_news_n_tri_list = file(resource_path('mag_news_n_list_tri_lemma_freq.csv'), 'rU').read()
			mag_news_n_tri_F = dict_builder(mag_news_n_tri_list, 2)

			mag_news_n_quad_list = file(resource_path('mag_news_n_list_quad_lemma_freq.csv'), 'rU').read()
			mag_news_n_quad_F = dict_builder(mag_news_n_quad_list, 2)

			#adjective
			mag_news_adj_bi_list = file(resource_path('mag_news_adj_list_bi_lemma_freq.csv'), 'rU').read()
			mag_news_adj_bi_F = dict_builder(mag_news_adj_bi_list, 2)

			mag_news_adj_tri_list = file(resource_path('mag_news_adj_list_tri_lemma_freq.csv'), 'rU').read()
			mag_news_adj_tri_F = dict_builder(mag_news_adj_tri_list, 2)

			mag_news_adj_quad_list = file(resource_path('mag_news_adj_list_quad_lemma_freq.csv'), 'rU').read()
			mag_news_adj_quad_F = dict_builder(mag_news_adj_quad_list, 2)

			#verb
			mag_news_v_bi_list = file(resource_path('mag_news_v_list_bi_lemma_freq.csv'), 'rU').read()
			mag_news_v_bi_F = dict_builder(mag_news_v_bi_list, 2)

			mag_news_v_tri_list = file(resource_path('mag_news_v_list_tri_lemma_freq.csv'), 'rU').read()
			mag_news_v_tri_F = dict_builder(mag_news_v_tri_list, 2)

			mag_news_v_quad_list = file(resource_path('mag_news_v_list_quad_lemma_freq.csv'), 'rU').read()
			mag_news_v_quad_F = dict_builder(mag_news_v_quad_list, 2)

			#verb_noun
			mag_news_v_n_bi_list = file(resource_path('mag_news_v_n_list_bi_lemma_freq.csv'), 'rU').read()
			mag_news_v_n_bi_F = dict_builder(mag_news_v_n_bi_list, 2)

			mag_news_v_n_tri_list = file(resource_path('mag_news_v_n_list_tri_lemma_freq.csv'), 'rU').read()
			mag_news_v_n_tri_F = dict_builder(mag_news_v_n_tri_list, 2)

			mag_news_v_n_quad_list = file(resource_path('mag_news_v_n_list_quad_lemma_freq.csv'), 'rU').read()
			mag_news_v_n_quad_F = dict_builder(mag_news_v_n_quad_list, 2)

			#adjective_noun
			mag_news_a_n_bi_list = file(resource_path('mag_news_a_n_list_bi_lemma_freq.csv'), 'rU').read()
			mag_news_a_n_bi_F = dict_builder(mag_news_a_n_bi_list, 2)

			mag_news_a_n_tri_list = file(resource_path('mag_news_a_n_list_tri_lemma_freq.csv'), 'rU').read()
			mag_news_a_n_tri_F = dict_builder(mag_news_a_n_tri_list, 2)

			mag_news_a_n_quad_list = file(resource_path('mag_news_a_n_list_quad_lemma_freq.csv'), 'rU').read()
		...
		#### Source Summary Analysis ####
			...
			source = file(source_text,"rU").read().lower()
